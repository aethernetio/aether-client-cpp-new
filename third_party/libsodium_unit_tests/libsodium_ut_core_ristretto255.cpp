...
namespace core_ristretto255
{
    ...
    tv3(void)
    {
        ...
        for (i = 0; i < 1000; i++) {
            ...
            if (memcmp(s, s_, crypto_core_ristretto255_BYTES) != 0) {
                DebugPrint("inversion failed\n");
            }
            if (crypto_scalarmult_ristretto255(s2, l, s2) == 0) {
                DebugPrint("s*l != inf (3)\n");
            }
            if (crypto_core_ristretto255_add(s2, s, s_) != 0) {
                DebugPrint("addition failed");
            }
            if (crypto_core_ristretto255_sub(s2, s2, s_) != 0) {
                DebugPrint("subtraction failed");
            }
            if (crypto_core_ristretto255_is_valid_point(s2) == 0) {
                DebugPrint("invalid point");
            }
            if (memcmp(s, s2, crypto_core_ristretto255_BYTES) != 0) {
                DebugPrint("s2 + s - s_ != s\n");
            }
            if (crypto_core_ristretto255_sub(s2, s2, s) != 0) {
                DebugPrint("subtraction failed");
            }
            if (crypto_core_ristretto255_is_valid_point(s2) == -1) {
                DebugPrint("s + s' - s - s' != 0");
            }
        }

        crypto_core_ristretto255_random(s);
        memset(s_, 0xfe, crypto_core_ristretto255_BYTES);
        TEST_ASSERT(crypto_core_ristretto255_add(s2, s_, s) == -1);
        TEST_ASSERT(crypto_core_ristretto255_add(s2, s, s_) == -1);
        TEST_ASSERT(crypto_core_ristretto255_add(s2, s_, s_) == -1);
        TEST_ASSERT(crypto_core_ristretto255_add(s2, s, s) == 0);
        TEST_ASSERT(crypto_core_ristretto255_sub(s2, s_, s) == -1);
        TEST_ASSERT(crypto_core_ristretto255_sub(s2, s, s_) == -1);
        TEST_ASSERT(crypto_core_ristretto255_sub(s2, s_, s_) == -1);
        TEST_ASSERT(crypto_core_ristretto255_sub(s2, s, s) == 0);
    ...
    tv4(void)
    {
        ...
        s1[0]--;
        TEST_ASSERT(sodium_is_zero(s1, crypto_core_ristretto255_SCALARBYTES));
    ...
    test(void)
    {
        ...
        TEST_ASSERT(crypto_core_ristretto255_BYTES == crypto_core_ristretto255_bytes());
        TEST_ASSERT(crypto_core_ristretto255_SCALARBYTES == crypto_core_ristretto255_scalarbytes());
        TEST_ASSERT(crypto_core_ristretto255_NONREDUCEDSCALARBYTES == crypto_core_ristretto255_nonreducedscalarbytes());
        TEST_ASSERT(crypto_core_ristretto255_NONREDUCEDSCALARBYTES >= crypto_core_ristretto255_SCALARBYTES);
        TEST_ASSERT(crypto_core_ristretto255_HASHBYTES == crypto_core_ristretto255_hashbytes());
        TEST_ASSERT(crypto_core_ristretto255_HASHBYTES >= crypto_core_ristretto255_BYTES);
        TEST_ASSERT(crypto_core_ristretto255_BYTES == crypto_core_ed25519_BYTES);
        TEST_ASSERT(crypto_core_ristretto255_SCALARBYTES == crypto_core_ed25519_SCALARBYTES);
        TEST_ASSERT(crypto_core_ristretto255_NONREDUCEDSCALARBYTES == crypto_core_ed25519_NONREDUCEDSCALARBYTES);
        TEST_ASSERT(crypto_core_ristretto255_HASHBYTES >= 2 * crypto_core_ed25519_UNIFORMBYTES);
    ...
    void _libsodium_ut_core_ristretto255()
    {
        test();
    }
