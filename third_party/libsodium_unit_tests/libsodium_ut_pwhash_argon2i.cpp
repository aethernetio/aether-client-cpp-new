...
namespace pwhash_argon2i
{
    ...
    tv2(void)
    {
        ...
        TEST_ASSERT(crypto_pwhash_argon2i(out, sizeof out, "password", strlen("password"), salt,
    ...
    tv3(void)
    {
        ...
        do {
            ...
            sodium_free(passwd);
            if (ret != 0) {
                DebugPrint("[tv3] pwhash_str failure (maybe intentional): [%u]\n",
                       (unsigned int) i);
            }
        } while (++i < (sizeof tests) / (sizeof tests[0]));
    }
    ...
    str_tests(void)
    {
        ...
        if (crypto_pwhash_argon2i_str(str_out, passwd, strlen(passwd), OPSLIMIT,
                                      MEMLIMIT) != 0) {
            DebugPrint("pwhash_argon2i_str failure\n");
        }
        if (crypto_pwhash_argon2i_str(str_out2, passwd, strlen(passwd), OPSLIMIT,
                                      MEMLIMIT) != 0) {
            DebugPrint("pwhash_argon2i_str(2) failure\n");
        }
        if (strcmp(str_out, str_out2) == 0) {
            DebugPrint("pwhash_argon2i_str() doesn't generate different salts\n");
        }
        ...
        if (crypto_pwhash_argon2i_str_needs_rehash(str_out, OPSLIMIT, MEMLIMIT / 2) != 1 ||
            ...
            crypto_pwhash_argon2i_str_needs_rehash(str_out, OPSLIMIT * 2, MEMLIMIT) != 1) {
            DebugPrint("needs_rehash() false negative\n");
        }
        if (crypto_pwhash_str_needs_rehash(str_out, OPSLIMIT, MEMLIMIT / 2) != 1) {
            DebugPrint("pwhash_str_needs_rehash() didn't handle argon2i\n");
        }
        ...
        if (sodium_is_zero((const unsigned char *) str_out + strlen(str_out),
            ...
            DebugPrint("pwhash_str() doesn't properly pad with zeros\n");
        }
        ...
        if (crypto_pwhash_argon2i_str_verify(str_out, passwd, strlen(passwd)) != -1) {
            DebugPrint("pwhash_str_verify(2) failure\n");
        }
        str_out[14]--;
        TEST_ASSERT(str_out[crypto_pwhash_STRBYTES - 1U] == 0);
        ...
        if (crypto_pwhash_argon2i_str(str_out2, passwd, strlen(passwd), 1, MEMLIMIT) !=
            -1) {
            DebugPrint("pwhash_str() with a small opslimit should have failed\n");
        }
        {
        ...
        if (crypto_pwhash_str_verify(
                ...
                "$TNnWIwlu1061JHrnCqIAmjs3huSxYIU+0jWipu7Kc9M",
                "password", strlen("password")) != -1 || errno != EINVAL) {
            DebugPrint("pwhash_str_verify(invalid(10)) failure\n");
        }
        if (crypto_pwhash_str_verify(
                "$argon2i$v=1$m=4096,t=3,p=2$b2RpZHVlamRpc29kaXNrdw"
                "$TNnWIwlu1061JHrnCqIAmjs3huSxYI~=U+0jWipu7Kc9M",
                "password", strlen("password")) != -1 || errno != EINVAL) {
            DebugPrint("pwhash_str_verify(invalid(11)) failure\n");
        }
        ...
        TEST_ASSERT(crypto_pwhash_argon2i_str_needs_rehash(str_out + 1,
                                                      OPSLIMIT, MEMLIMIT) == -1);
        TEST_ASSERT(crypto_pwhash_argon2id_str_needs_rehash(str_out, 0, 0) == -1);
        TEST_ASSERT(crypto_pwhash_argon2id_str_needs_rehash("", OPSLIMIT, MEMLIMIT) == -1);
        TEST_ASSERT(crypto_pwhash_str_alg(str_out, "test", 4, OPSLIMIT, MEMLIMIT,
    ...
    test(void)
    {
        ...
        TEST_ASSERT(crypto_pwhash_argon2i_bytes_min() > 0U);
        TEST_ASSERT(crypto_pwhash_argon2i_bytes_max() > crypto_pwhash_argon2i_bytes_min());
        TEST_ASSERT(crypto_pwhash_argon2i_passwd_max() > crypto_pwhash_argon2i_passwd_min());
        TEST_ASSERT(crypto_pwhash_argon2i_saltbytes() > 0U);
        TEST_ASSERT(crypto_pwhash_argon2i_strbytes() > 1U);
        TEST_ASSERT(crypto_pwhash_argon2i_strbytes() > strlen(crypto_pwhash_argon2i_strprefix()));

        TEST_ASSERT(crypto_pwhash_argon2i_opslimit_min() > 0U);
        TEST_ASSERT(crypto_pwhash_argon2i_opslimit_max() > 0U);
        TEST_ASSERT(crypto_pwhash_argon2i_memlimit_min() > 0U);
        TEST_ASSERT(crypto_pwhash_argon2i_memlimit_max() > 0U);
        TEST_ASSERT(crypto_pwhash_argon2i_opslimit_interactive() > 0U);
        TEST_ASSERT(crypto_pwhash_argon2i_memlimit_interactive() > 0U);
        TEST_ASSERT(crypto_pwhash_argon2i_opslimit_moderate() > 0U);
        TEST_ASSERT(crypto_pwhash_argon2i_memlimit_moderate() > 0U);
        TEST_ASSERT(crypto_pwhash_argon2i_opslimit_sensitive() > 0U);
        TEST_ASSERT(crypto_pwhash_argon2i_memlimit_sensitive() > 0U);

        TEST_ASSERT(crypto_pwhash_argon2i_bytes_min() == crypto_pwhash_argon2i_BYTES_MIN);
        TEST_ASSERT(crypto_pwhash_argon2i_bytes_max() == crypto_pwhash_argon2i_BYTES_MAX);
        TEST_ASSERT(crypto_pwhash_argon2i_passwd_min() == crypto_pwhash_argon2i_PASSWD_MIN);
        TEST_ASSERT(crypto_pwhash_argon2i_passwd_max() == crypto_pwhash_argon2i_PASSWD_MAX);
        TEST_ASSERT(crypto_pwhash_argon2i_saltbytes() == crypto_pwhash_argon2i_SALTBYTES);
        TEST_ASSERT(crypto_pwhash_argon2i_strbytes() == crypto_pwhash_argon2i_STRBYTES);

        TEST_ASSERT(crypto_pwhash_argon2i_opslimit_min() == crypto_pwhash_argon2i_OPSLIMIT_MIN);
        TEST_ASSERT(crypto_pwhash_argon2i_opslimit_max() == crypto_pwhash_argon2i_OPSLIMIT_MAX);
        TEST_ASSERT(crypto_pwhash_argon2i_memlimit_min() == crypto_pwhash_argon2i_MEMLIMIT_MIN);
        TEST_ASSERT(crypto_pwhash_argon2i_memlimit_max() == crypto_pwhash_argon2i_MEMLIMIT_MAX);
        TEST_ASSERT(crypto_pwhash_argon2i_opslimit_interactive() ==
               crypto_pwhash_argon2i_OPSLIMIT_INTERACTIVE);
        TEST_ASSERT(crypto_pwhash_argon2i_memlimit_interactive() ==
               crypto_pwhash_argon2i_MEMLIMIT_INTERACTIVE);
        TEST_ASSERT(crypto_pwhash_argon2i_opslimit_moderate() ==
               crypto_pwhash_argon2i_OPSLIMIT_MODERATE);
        TEST_ASSERT(crypto_pwhash_argon2i_memlimit_moderate() ==
               crypto_pwhash_argon2i_MEMLIMIT_MODERATE);
        TEST_ASSERT(crypto_pwhash_argon2i_opslimit_sensitive() ==
               crypto_pwhash_argon2i_OPSLIMIT_SENSITIVE);
        TEST_ASSERT(crypto_pwhash_argon2i_memlimit_sensitive() ==
               crypto_pwhash_argon2i_MEMLIMIT_SENSITIVE);

        TEST_ASSERT(crypto_pwhash_argon2i_alg_argon2i13() == crypto_pwhash_argon2i_ALG_ARGON2I13);
