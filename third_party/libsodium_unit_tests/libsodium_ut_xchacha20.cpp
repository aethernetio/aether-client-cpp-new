...
namespace xchacha20
{
    ...
    tv_hchacha20(void)
    {
        ...
        for (i = 0; i < (sizeof tvs) / (sizeof tvs[0]); i++) {
            ...
            TEST_ASSERT(memcmp(out, out2, crypto_core_hchacha20_OUTPUTBYTES) == 0);
        }
        ...
        TEST_ASSERT(crypto_core_hchacha20_outputbytes() == crypto_core_hchacha20_OUTPUTBYTES);
        TEST_ASSERT(crypto_core_hchacha20_inputbytes() == crypto_core_hchacha20_INPUTBYTES);
        TEST_ASSERT(crypto_core_hchacha20_keybytes() == crypto_core_hchacha20_KEYBYTES);
        TEST_ASSERT(crypto_core_hchacha20_constbytes() == crypto_core_hchacha20_CONSTBYTES);

        DebugPrint("tv_hchacha20: ok\n");
    }
    ...
    tv_stream_xchacha20(void)
    {
        ...
        for (i = 0; i < (sizeof tvs) / (sizeof tvs[0]); i++) {
            ...
            TEST_ASSERT(memcmp(out, out2, out_len) == 0);
            crypto_stream_xchacha20_xor(out2, out, out_len, nonce, key);
            TEST_ASSERT(sodium_is_zero(out2, out_len));
            ...
            TEST_ASSERT(sodium_is_zero(out, out_len));
            sodium_free(out2);
        }
        ...
        TEST_ASSERT(memcmp(out, out2 + 64, 64) == 0);
        sodium_free(out);
        sodium_free(out2);
        ...
        crypto_stream_xchacha20_xor_ic(out + 128, out + 128, 64, nonce,
                                       (1ULL << 32) + 1, key);
        TEST_ASSERT(memcmp(out, out2, 192) == 0);
        ...
        TEST_ASSERT(sodium_is_zero(key, crypto_stream_xchacha20_KEYBYTES) == 0);
        ...
        TEST_ASSERT(crypto_stream_xchacha20_keybytes() == crypto_stream_xchacha20_KEYBYTES);
        TEST_ASSERT(crypto_stream_xchacha20_noncebytes() == crypto_stream_xchacha20_NONCEBYTES);
        TEST_ASSERT(crypto_stream_xchacha20_messagebytes_max() == crypto_stream_xchacha20_MESSAGEBYTES_MAX);

        DebugPrint("tv_stream_xchacha20: ok\n");
    }
    ...
    tv_secretbox_xchacha20poly1305(void)
    {
        ...
        for (i = 0; i < (sizeof tvs) / (sizeof tvs[0]); i++) {
            ...
            sodium_hex2bin(out, crypto_secretbox_xchacha20poly1305_MACBYTES + m_len,
                           tv->out, strlen(tv->out), NULL, NULL, NULL);
            TEST_ASSERT(crypto_secretbox_xchacha20poly1305_easy(out2, m, 0, nonce, key) == 0);
            TEST_ASSERT(crypto_secretbox_xchacha20poly1305_easy(out2, m, m_len, nonce, key) == 0);
            TEST_ASSERT(memcmp(out, out2,
            ...
            TEST_ASSERT(crypto_secretbox_xchacha20poly1305_open_easy
            ...
            TEST_ASSERT(memcmp(m, out2, m_len) == 0);
            TEST_ASSERT(crypto_secretbox_xchacha20poly1305_open_detached
        ...
        TEST_ASSERT(crypto_secretbox_xchacha20poly1305_keybytes() == crypto_secretbox_xchacha20poly1305_KEYBYTES);
        TEST_ASSERT(crypto_secretbox_xchacha20poly1305_noncebytes() == crypto_secretbox_xchacha20poly1305_NONCEBYTES);
        TEST_ASSERT(crypto_secretbox_xchacha20poly1305_macbytes() == crypto_secretbox_xchacha20poly1305_MACBYTES);
        TEST_ASSERT(crypto_secretbox_xchacha20poly1305_messagebytes_max() == crypto_secretbox_xchacha20poly1305_MESSAGEBYTES_MAX);

        DebugPrint("tv_secretbox_xchacha20: ok\n");
    }
    ...
    tv_box_xchacha20poly1305(void)
    {
        ...
        for (i = 0; i < 10; i++) {
            ...
            TEST_ASSERT(crypto_box_curve25519xchacha20poly1305_keypair(pk, sk) == 0);
            TEST_ASSERT(crypto_box_curve25519xchacha20poly1305_easy(out, m, 0, nonce,
                                                               pk, sk) == 0);
            TEST_ASSERT(crypto_box_curve25519xchacha20poly1305_easy(out, m, m_len, nonce,
                                                               pk, sk) == 0);
            TEST_ASSERT(crypto_box_curve25519xchacha20poly1305_open_easy
            ...
            TEST_ASSERT(crypto_box_curve25519xchacha20poly1305_open_easy
                   (m2, out, crypto_box_curve25519xchacha20poly1305_MACBYTES + m_len,
                    nonce, pk, sk) == 0);
            TEST_ASSERT(memcmp(m2, m, m_len) == 0);
            sodium_free(out);

            out = (unsigned char *) sodium_malloc
                (crypto_box_curve25519xchacha20poly1305_MACBYTES + m_len);
            TEST_ASSERT(crypto_box_curve25519xchacha20poly1305_beforenm(pc, small_order_p, sk) == -1);
            TEST_ASSERT(crypto_box_curve25519xchacha20poly1305_beforenm(pc, pk, sk) == 0);
            TEST_ASSERT(crypto_box_curve25519xchacha20poly1305_easy_afternm
                   (out, m, 0, nonce, pc) == 0);
            TEST_ASSERT(crypto_box_curve25519xchacha20poly1305_easy_afternm
                   (out, m, m_len, nonce, pc) == 0);
            TEST_ASSERT(crypto_box_curve25519xchacha20poly1305_open_easy_afternm
                   (m2, out, crypto_box_curve25519xchacha20poly1305_MACBYTES - 1,
                    nonce, pc) == -1);
            TEST_ASSERT(crypto_box_curve25519xchacha20poly1305_open_easy_afternm
                   (m2, out, 0,
            ...
            TEST_ASSERT(crypto_box_curve25519xchacha20poly1305_detached(out, mac, m, m_len,
                                                                   nonce, small_order_p, sk) == -1);
            TEST_ASSERT(crypto_box_curve25519xchacha20poly1305_detached(out, mac, m, m_len,
                                                                   nonce, pk, sk) == 0);
            TEST_ASSERT(crypto_box_curve25519xchacha20poly1305_open_detached
            ...
            out = (unsigned char *) sodium_malloc(m_len);
            TEST_ASSERT(crypto_box_curve25519xchacha20poly1305_detached_afternm
                   (out, mac, m, m_len, nonce, pc) == 0);
            TEST_ASSERT(crypto_box_curve25519xchacha20poly1305_open_detached_afternm
        ...
        TEST_ASSERT(strcmp(hex, "4701d08488451f545a409fb58ae3e58581ca40ac3f7f114698cd71deac73ca01") == 0);
        sodium_bin2hex(hex, sizeof hex, sk, crypto_box_curve25519xchacha20poly1305_SECRETKEYBYTES);
        TEST_ASSERT(strcmp(hex, "3d94eea49c580aef816935762be049559d6d1440dede12e6a125f1841fff8e6f") == 0);
        sodium_free(seed);

        sodium_free(sk);
        sodium_free(pk);

        TEST_ASSERT(crypto_box_curve25519xchacha20poly1305_seedbytes() == crypto_box_curve25519xchacha20poly1305_SEEDBYTES);
        TEST_ASSERT(crypto_box_curve25519xchacha20poly1305_publickeybytes() == crypto_box_curve25519xchacha20poly1305_PUBLICKEYBYTES);
        TEST_ASSERT(crypto_box_curve25519xchacha20poly1305_secretkeybytes() == crypto_box_curve25519xchacha20poly1305_SECRETKEYBYTES);
        TEST_ASSERT(crypto_box_curve25519xchacha20poly1305_beforenmbytes() == crypto_box_curve25519xchacha20poly1305_BEFORENMBYTES);
        TEST_ASSERT(crypto_box_curve25519xchacha20poly1305_noncebytes() == crypto_box_curve25519xchacha20poly1305_NONCEBYTES);
        TEST_ASSERT(crypto_box_curve25519xchacha20poly1305_macbytes() == crypto_box_curve25519xchacha20poly1305_MACBYTES);
        TEST_ASSERT(crypto_box_curve25519xchacha20poly1305_messagebytes_max() == crypto_box_curve25519xchacha20poly1305_MESSAGEBYTES_MAX);

        DebugPrint("tv_box_xchacha20poly1305: ok\n");
    }
