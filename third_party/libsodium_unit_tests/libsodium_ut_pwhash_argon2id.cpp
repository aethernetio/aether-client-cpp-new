...
namespace pwhash_argon2id
{
    ...
    tv(void)
    {
        ...
        do {
            ...
            if (crypto_pwhash(out, (unsigned long long) tests[i].outlen, passwd,
                ...
                DebugPrint("[tv] pwhash failure (maybe intentional): [%u]\n",
                       (unsigned int) i);
                continue;
            }
    ...
    tv2(void)
    {
        ...
        do {
            ...
            if (crypto_pwhash(out, (unsigned long long) tests[i].outlen, passwd,
                              ...
                              tests[i].opslimit, tests[i].memlimit,
                              crypto_pwhash_alg_default()) != 0) {
                DebugPrint("[tv2] pwhash failure: [%u]\n", (unsigned int) i);
                continue;
            }
            sodium_bin2hex(out_hex, sizeof out_hex, out, tests[i].outlen);
            DebugPrint("%s\n", out_hex);
        ...
        if (crypto_pwhash_argon2id(out, sizeof out, "password", strlen("password"), salt, 3,
                                   1ULL << 12, 0) != -1) {
            DebugPrint("[tv2] pwhash should have failed (0)\n");
        }
        if (crypto_pwhash_argon2id(out, sizeof out, "password", strlen("password"), salt, 3,
                                   1, crypto_pwhash_argon2id_alg_argon2id13()) != -1) {
            DebugPrint("[tv2] pwhash should have failed (1)\n");
        }
        if (crypto_pwhash_argon2id(out, sizeof out, "password", strlen("password"), salt, 3,
                                   1ULL << 12, crypto_pwhash_argon2id_alg_argon2id13()) != -1) {
            DebugPrint("[tv2] pwhash should have failed (2)\n");
        }
        if (crypto_pwhash_argon2id(out, sizeof out, "password", strlen("password"), salt, 2,
                                   1ULL << 12, crypto_pwhash_argon2id_alg_argon2id13()) != -1) {
            DebugPrint("[tv2] pwhash should have failed (3)\n");
        }
        if (crypto_pwhash_argon2id(out, 15, "password", strlen("password"), salt, 3,
                                   1ULL << 12, crypto_pwhash_argon2id_alg_argon2id13()) != -1) {
            DebugPrint("[tv2] pwhash with a short output length should have failed\n");
        }
        if (crypto_pwhash_argon2id(out, sizeof out, "password", 0x100000000ULL, salt, 3,
                                   1ULL << 12, crypto_pwhash_argon2id_alg_argon2id13()) != -1) {
            DebugPrint("[tv2] pwhash with a long password length should have failed\n");
        }
        TEST_ASSERT(crypto_pwhash_argon2id(out, sizeof out, "password", strlen("password"), salt,
    ...
    str_tests(void)
    {
        ...
        if (crypto_pwhash_str_needs_rehash(str_out, OPSLIMIT, MEMLIMIT / 2) != 1 ||
            ...
            crypto_pwhash_str_needs_rehash(str_out, OPSLIMIT + 1, MEMLIMIT) != 1) {
            DebugPrint("needs_rehash() false negative (0)\n");
        }
        if (crypto_pwhash_argon2id_str_needs_rehash(str_out, OPSLIMIT, MEMLIMIT / 2) != 1 ||
            ...
            crypto_pwhash_argon2id_str_needs_rehash(str_out, OPSLIMIT + 1, MEMLIMIT) != 1) {
            DebugPrint("needs_rehash() false negative (1)\n");
        }
        if (crypto_pwhash_argon2i_str_needs_rehash(str_out, OPSLIMIT, MEMLIMIT / 2) != -1 ||
            ...
            crypto_pwhash_argon2i_str_needs_rehash(str_out, OPSLIMIT + 1, MEMLIMIT) != -1) {
            DebugPrint("needs_rehash() false negative (2)\n");
        }
        if (crypto_pwhash_str_needs_rehash(str_out, OPSLIMIT, MEMLIMIT / 2) != 1) {
            DebugPrint("pwhash_str_needs_rehash() didn't handle argon2id\n");
        }
        if (crypto_pwhash_str_needs_rehash(str_out + 1, OPSLIMIT, MEMLIMIT) != -1 ||
            ...
            DebugPrint("needs_rehash() didn't fail with an invalid hash string\n");
        }
        if (sodium_is_zero((const unsigned char *) str_out + strlen(str_out),
            ...
            DebugPrint("pwhash_argon2id_str() doesn't properly pad with zeros\n");
        }
        if (crypto_pwhash_argon2id_str_verify(str_out, passwd, strlen(passwd)) != 0) {
            DebugPrint("pwhash_argon2id_str_verify(1) failure\n");
        }
        if (crypto_pwhash_str_verify(str_out, passwd, strlen(passwd)) != 0) {
            DebugPrint("pwhash_str_verify(1') failure\n");
        }
        str_out[14]++;
        if (crypto_pwhash_str_verify(str_out, passwd, strlen(passwd)) != -1) {
            DebugPrint("pwhash_argon2id_str_verify(2) failure\n");
        }
        str_out[14]--;
        TEST_ASSERT(str_out[crypto_pwhash_argon2id_STRBYTES - 1U] == 0);

        if (crypto_pwhash_str(str_out2, passwd, 0x100000000ULL, OPSLIMIT,
                              MEMLIMIT) != -1) {
            DebugPrint("pwhash_str() with a large password should have failed\n");
        }
        if (crypto_pwhash_str(str_out2, passwd, strlen(passwd), 1, MEMLIMIT) != 0) {
            DebugPrint("pwhash_str() with a small opslimit should not have failed\n");
        }
        if (crypto_pwhash_str(str_out2, passwd, strlen(passwd), 0, MEMLIMIT) != -1) {
            DebugPrint("pwhash_argon2id_str() with a null opslimit should have failed\n");
        }
        {
            ...
            if (crypto_pwhash_argon2i_str_verify(str_in, password_in,
                                                 0x100000000ULL) != -1) {
                DebugPrint("pwhash_str_verify(invalid(0)) failure\n");
            }
        ...
        if (crypto_pwhash_str_verify("$argon2id$m=65536,t=2,p=1c29tZXNhbHQ"
            ...
            DebugPrint("pwhash_str_verify(invalid(1)) failure %d\n", errno);
        }
        if (crypto_pwhash_str_verify("$argon2id$m=65536,t=2,p=1$c29tZXNhbHQ"
                                     "9sTbSlTio3Biev89thdrlKKiCaYsjjYVJxGAL3swxpQ",
                                     "password", strlen("password")) != -1) {
            DebugPrint("pwhash_str_verify(invalid(2)) failure\n");
        }
        if (crypto_pwhash_str_verify("$argon2id$m=65536,t=2,p=1$c29tZXNhbHQ"
                                     "$b2G3seW+uPzerwQQC+/E1K50CLLO7YXy0JRcaTuswRo",
                                     "password", strlen("password")) != -1) {
            DebugPrint("pwhash_str_verify(invalid(3)) failure\n");
        }
        if (crypto_pwhash_str_verify("$argon2id$v=19$m=65536,t=2,p=1c29tZXNhbHQ"
                                     "$wWKIMhR9lyDFvRz9YTZweHKfbftvj+qf+YFY4NeBbtA",
                                     "password", strlen("password")) != -1) {
            DebugPrint("pwhash_str_verify(invalid(4)) failure\n");
        }
        if (crypto_pwhash_str_verify("$argon2id$v=19$m=65536,t=2,p=1$c29tZXNhbHQ"
                                     "wWKIMhR9lyDFvRz9YTZweHKfbftvj+qf+YFY4NeBbtA",
                                     "password", strlen("password")) != -1) {
            DebugPrint("pwhash_str_verify(invalid(5)) failure\n");
        }
        if (crypto_pwhash_str_verify("$argon2id$v=19$m=65536,t=2,p=1$c29tZXNhbHQ"
                                     "$8iIuixkI73Js3G1uMbezQXD0b8LG4SXGsOwoQkdAQIM",
                                     "password", strlen("password")) != -1) {
            DebugPrint("pwhash_str_verify(invalid(6)) failure\n");
        }
        if (crypto_pwhash_str_verify("$argon2id$v=19$m=256,t=3,p=1$MDEyMzQ1Njc"
                                     "$G5ajKFCoUzaXRLdz7UJb5wGkb2Xt+X5/GQjUYtS2+TE",
                                     "password", strlen("password")) != 0) {
            DebugPrint("pwhash_str_verify(valid(7)) failure\n");
        }
        if (crypto_pwhash_str_verify("$argon2id$v=19$m=256,t=3,p=1$MDEyMzQ1Njc"
                                     "$G5ajKFCoUzaXRLdz7UJb5wGkb2Xt+X5/GQjUYtS2+TE",
                                     "passwore", strlen("passwore")) != -1 || errno != EINVAL) {
            DebugPrint("pwhash_str_verify(invalid(7)) failure\n");
        }
        if (crypto_pwhash_str_verify("$Argon2id$v=19$m=256,t=3,p=1$MDEyMzQ1Njc"
                                     "$G5ajKFCoUzaXRLdz7UJb5wGkb2Xt+X5/GQjUYtS2+TE",
                                     "password", strlen("password")) != -1 || errno != EINVAL) {
            DebugPrint("pwhash_str_verify(invalid(8)) failure\n");
        }
        if (crypto_pwhash_str_verify("$argon2id$v=19$m=256,t=3,p=2$MDEyMzQ1Njc"
                                     "$G5ajKFCoUzaXRLdz7UJb5wGkb2Xt+X5/GQjUYtS2+TE",
                                     "password", strlen("password")) != -1 || errno != EINVAL) {
            DebugPrint("pwhash_str_verify(invalid(9)) failure\n");
        }
        TEST_ASSERT(crypto_pwhash_str_alg(str_out, "test", 4, OPSLIMIT, MEMLIMIT,
                                     crypto_pwhash_ALG_ARGON2ID13) == 0);
        TEST_ASSERT(crypto_pwhash_argon2id_str_verify(str_out, "test", 4) == 0);
        TEST_ASSERT(crypto_pwhash_argon2id_str_needs_rehash(str_out,
                                                       OPSLIMIT, MEMLIMIT) == 0);
        TEST_ASSERT(crypto_pwhash_argon2id_str_needs_rehash(str_out,
                                                       OPSLIMIT / 2, MEMLIMIT) == 1);
        TEST_ASSERT(crypto_pwhash_argon2id_str_needs_rehash(str_out,
                                                       OPSLIMIT, MEMLIMIT / 2) == 1);
        TEST_ASSERT(crypto_pwhash_argon2id_str_needs_rehash(str_out, 0, 0) == 1);
        TEST_ASSERT(crypto_pwhash_argon2i_str_needs_rehash(str_out, 0, 0) == -1);
        TEST_ASSERT(crypto_pwhash_argon2id_str_needs_rehash(str_out + 1,
                                                       OPSLIMIT, MEMLIMIT) == -1);
        TEST_ASSERT(crypto_pwhash_argon2i_str_needs_rehash(str_out, 0, 0) == -1);
        TEST_ASSERT(crypto_pwhash_argon2i_str_needs_rehash("", OPSLIMIT, MEMLIMIT) == -1);
        TEST_ASSERT(crypto_pwhash_str_alg(str_out, "test", 4, OPSLIMIT, MEMLIMIT,
                                     crypto_pwhash_ALG_ARGON2I13) == 0);
        TEST_ASSERT(crypto_pwhash_argon2i_str_verify(str_out, "test", 4) == 0);
        TEST_ASSERT(crypto_pwhash_argon2i_str_needs_rehash(str_out,
                                                      OPSLIMIT, MEMLIMIT) == 0);
        TEST_ASSERT(crypto_pwhash_argon2i_str_needs_rehash(str_out,
                                                      OPSLIMIT / 2, MEMLIMIT) == 1);
        TEST_ASSERT(crypto_pwhash_argon2i_str_needs_rehash(str_out,
        ...
        TEST_ASSERT(crypto_pwhash_argon2id_str_needs_rehash(str_out, 0, 0) == -1);
        TEST_ASSERT(crypto_pwhash_argon2i_str_needs_rehash("", OPSLIMIT, MEMLIMIT) == -1);
        TEST_ASSERT(crypto_pwhash_argon2i_str_needs_rehash(str_out + 1,
                                                      OPSLIMIT, MEMLIMIT) == -1);
        sodium_free(salt);
        sodium_free(str_out);
    ...
    test(void)
    {
        ...
        TEST_ASSERT(crypto_pwhash_bytes_min() > 0U);
        TEST_ASSERT(crypto_pwhash_bytes_max() > crypto_pwhash_bytes_min());
        TEST_ASSERT(crypto_pwhash_passwd_max() > crypto_pwhash_passwd_min());
        TEST_ASSERT(crypto_pwhash_saltbytes() > 0U);
        TEST_ASSERT(crypto_pwhash_strbytes() > 1U);
        TEST_ASSERT(crypto_pwhash_strbytes() > strlen(crypto_pwhash_strprefix()));

        TEST_ASSERT(crypto_pwhash_opslimit_min() > 0U);
        TEST_ASSERT(crypto_pwhash_opslimit_max() > 0U);
        TEST_ASSERT(crypto_pwhash_memlimit_min() > 0U);
        TEST_ASSERT(crypto_pwhash_memlimit_max() > 0U);
        TEST_ASSERT(crypto_pwhash_opslimit_interactive() > 0U);
        TEST_ASSERT(crypto_pwhash_memlimit_interactive() > 0U);
        TEST_ASSERT(crypto_pwhash_opslimit_moderate() > 0U);
        TEST_ASSERT(crypto_pwhash_memlimit_moderate() > 0U);
        TEST_ASSERT(crypto_pwhash_opslimit_sensitive() > 0U);
        TEST_ASSERT(crypto_pwhash_memlimit_sensitive() > 0U);
        TEST_ASSERT(strcmp(crypto_pwhash_primitive(), "argon2id,argon2i") == 0);

        TEST_ASSERT(crypto_pwhash_bytes_min() == crypto_pwhash_BYTES_MIN);
        TEST_ASSERT(crypto_pwhash_bytes_max() == crypto_pwhash_BYTES_MAX);
        TEST_ASSERT(crypto_pwhash_passwd_min() == crypto_pwhash_PASSWD_MIN);
        TEST_ASSERT(crypto_pwhash_passwd_max() == crypto_pwhash_PASSWD_MAX);
        TEST_ASSERT(crypto_pwhash_saltbytes() == crypto_pwhash_SALTBYTES);
        TEST_ASSERT(crypto_pwhash_strbytes() == crypto_pwhash_STRBYTES);

        TEST_ASSERT(crypto_pwhash_opslimit_min() == crypto_pwhash_OPSLIMIT_MIN);
        TEST_ASSERT(crypto_pwhash_opslimit_max() == crypto_pwhash_OPSLIMIT_MAX);
        TEST_ASSERT(crypto_pwhash_memlimit_min() == crypto_pwhash_MEMLIMIT_MIN);
        TEST_ASSERT(crypto_pwhash_memlimit_max() == crypto_pwhash_MEMLIMIT_MAX);
        TEST_ASSERT(crypto_pwhash_opslimit_interactive() ==
               crypto_pwhash_OPSLIMIT_INTERACTIVE);
        TEST_ASSERT(crypto_pwhash_memlimit_interactive() ==
               crypto_pwhash_MEMLIMIT_INTERACTIVE);
        TEST_ASSERT(crypto_pwhash_opslimit_moderate() ==
               crypto_pwhash_OPSLIMIT_MODERATE);
        TEST_ASSERT(crypto_pwhash_memlimit_moderate() ==
               crypto_pwhash_MEMLIMIT_MODERATE);
        TEST_ASSERT(crypto_pwhash_opslimit_sensitive() ==
               crypto_pwhash_OPSLIMIT_SENSITIVE);
        TEST_ASSERT(crypto_pwhash_memlimit_sensitive() ==
               crypto_pwhash_MEMLIMIT_SENSITIVE);

        TEST_ASSERT(crypto_pwhash_argon2id_bytes_min() == crypto_pwhash_bytes_min());
        TEST_ASSERT(crypto_pwhash_argon2id_bytes_max() == crypto_pwhash_bytes_max());
        TEST_ASSERT(crypto_pwhash_argon2id_passwd_min() == crypto_pwhash_passwd_min());
        TEST_ASSERT(crypto_pwhash_argon2id_passwd_max() == crypto_pwhash_passwd_max());
        TEST_ASSERT(crypto_pwhash_argon2id_saltbytes() == crypto_pwhash_saltbytes());
        TEST_ASSERT(crypto_pwhash_argon2id_strbytes() == crypto_pwhash_strbytes());
        TEST_ASSERT(strcmp(crypto_pwhash_argon2id_strprefix(),
                      crypto_pwhash_strprefix()) == 0);
        TEST_ASSERT(crypto_pwhash_argon2id_opslimit_min() ==
               crypto_pwhash_opslimit_min());
        TEST_ASSERT(crypto_pwhash_argon2id_opslimit_max() ==
               crypto_pwhash_opslimit_max());
        TEST_ASSERT(crypto_pwhash_argon2id_memlimit_min() ==
               crypto_pwhash_memlimit_min());
        TEST_ASSERT(crypto_pwhash_argon2id_memlimit_max() ==
               crypto_pwhash_memlimit_max());
        TEST_ASSERT(crypto_pwhash_argon2id_opslimit_interactive() ==
               crypto_pwhash_opslimit_interactive());
        TEST_ASSERT(crypto_pwhash_argon2id_opslimit_moderate() ==
               crypto_pwhash_opslimit_moderate());
        TEST_ASSERT(crypto_pwhash_argon2id_opslimit_sensitive() ==
               crypto_pwhash_opslimit_sensitive());
        TEST_ASSERT(crypto_pwhash_argon2id_memlimit_interactive() ==
               crypto_pwhash_memlimit_interactive());
        TEST_ASSERT(crypto_pwhash_argon2id_memlimit_moderate() ==
               crypto_pwhash_memlimit_moderate());
        TEST_ASSERT(crypto_pwhash_argon2id_memlimit_sensitive() ==
               crypto_pwhash_memlimit_sensitive());
        TEST_ASSERT(crypto_pwhash_alg_argon2id13() ==
               crypto_pwhash_argon2id_alg_argon2id13());
        TEST_ASSERT(crypto_pwhash_alg_argon2i13() == crypto_pwhash_ALG_ARGON2I13);
        TEST_ASSERT(crypto_pwhash_alg_argon2i13() != crypto_pwhash_alg_default());
        TEST_ASSERT(crypto_pwhash_alg_argon2id13() == crypto_pwhash_ALG_ARGON2ID13);
        TEST_ASSERT(crypto_pwhash_alg_argon2id13() != crypto_pwhash_alg_argon2i13());
        TEST_ASSERT(crypto_pwhash_alg_argon2id13() == crypto_pwhash_alg_default());

        TEST_ASSERT(crypto_pwhash_argon2id(guard_page, 0, (const char *) guard_page, 0, guard_page,
        ...
        TEST_ASSERT(crypto_pwhash_argon2i(guard_page, 0, (const char *) guard_page, 0, guard_page,
                                     crypto_pwhash_argon2id_OPSLIMIT_INTERACTIVE,
                                     crypto_pwhash_argon2id_MEMLIMIT_INTERACTIVE,
                                     0) == -1);
        TEST_ASSERT(crypto_pwhash_argon2i(guard_page, 0, (const char *) guard_page, 0, guard_page,
